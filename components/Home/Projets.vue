<template>
    <div class="lg:mt-32 md:mt-0 sm:mt-52 tablet">
        ...
        <div v-else class="container mx-auto grid xl:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 gap-10 md:gap-20 xl:gap-x-52 md:mt-32 sm:-mt-2">
            <div class="mx-auto card" v-for="projet in projets.slice(0, 4)" :key="projet.item" data-aos="fade-up">
                <NuxtLink :to="'projet/' + projet.attributes.Slug">
                    <img alt="Image illustration projet" class="img-fluid w-full"
                        :src="apiUrl + projet.attributes.Image.data.attributes.formats.small.url">
                    <div class="p-5">
                        <h2 class="xl:text-2xl md:text-base sm:text-l mt-1 textSize">{{ projet.attributes.Title }}</h2>
